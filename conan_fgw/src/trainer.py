...
class TrainerHolder:
    ...
    @staticmethod
    def regression_metric_name(squared: bool = True):
        """
        Returns:
    ...
    @staticmethod
    def regression_metric(predicted, expected, squared: bool = True):
        """
    ...
    def create_trainer(self, run_name: str, verbose: bool = False) -> pl.Trainer:
        """
        ...
        Args:
            ...
            verbose (bool): Print detail running.
        Returns:
        ...
        if "Classification" in self.config.experiment.model_class.__name__:
            ...
            task = "regression"
        callbacks = [
            self.early_stopping_callback(use_loss=True, verbose=verbose),
            self.checkpoint_callback(run_name, task=task, verbose=verbose),
            self.lr_monitor,
            self.timer,
        ]
        return pl.Trainer(
            ...
            enable_progress_bar=verbose,  # Disable the progress bar
            enable_model_summary=verbose,  # Disable the model summary
        )
    ...
    def early_stopping_callback(
        self, use_loss: bool = False, verbose: bool = False
    ) -> EarlyStopping:
        """
    ...
    def checkpoint_callback(
        self, run_name: str, task: str = "regression", verbose: bool = False
    ) -> ModelCheckpoint:
        """
        ...
        if task == "regression":
            if TrainerHolder.regression_metric_name() == "rmse":
                filename = "{epoch}-{step}-{val_rmse:.2f}"
            elif TrainerHolder.regression_metric_name() == "mse":
                filename = "{epoch}-{step}-{val_mse:.2f}"
            else:
                raise NotImplementedError(
                    f"Metric '{TrainerHolder.regression_metric_name()}' is not implemented."
                )
            return ModelCheckpoint(
                dirpath=dirpath,  # Directory where the checkpoints will be saved
                filename=filename,  # Checkpoint file name format
                verbose=verbose,  # Print a message when a new best checkpoint is saved
